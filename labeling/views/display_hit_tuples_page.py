...
class DisplayHitTuplesPage(View):
    ...
    def generate_hit(self, project, worker):
         ...
         if not hit:
             # generate a new hit
             res = Table.objects.filter(Q(project = project), Q(complete_indicator = False))
             ...
             Table.objects.filter(id__in = idx).update(complete_indicator = True)
    ...
    def generate_assignment(self, hit, project, worker):
         ...
         assignment = Assignment(hit = hit, worker = worker,
                ...
                # assignment expiry days is not in use right now, need to be modified if you are to use expiry_time for assignments.
         assignment.save()
